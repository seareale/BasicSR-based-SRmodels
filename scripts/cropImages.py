...
def main():
    ...
    opt['save_folder'] = f'datasets/{save_folder}/train/hr'
    opt['crop_size'] = 256 # 480
    opt['step'] = 128 # 240
    opt['thresh_size'] = 0
    opt['offset'] = offset
...
def worker(path, opt):
    ...
    for x in h_space:
        for y in w_space:
            ...
            cv2.imwrite(
                osp.join(opt['save_folder'], f"{opt['offset']+'_'}{img_name}_s{index:03d}{extension}"), cropped_img,
